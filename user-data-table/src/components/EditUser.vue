<template>
  <div @keyup.esc="closeEditModal" class="modal-container">
    <div class="modal">
      <button
...
export default defineComponent({
  ...
  methods: {
     ...
     * Create object to send to the API
     * Send object to the API
     * Edit object in local DataBase.
     */
    edit(): void {
      ...
      this.phoneNumberError =
        ...
        this.phoneNumber.toString().length > this.phoneNumberLen
          ? `Phone number should have at least ${this.phoneNumberLen} digits`
      ...
      if (
        ...
        // Clean API object
        for (const key in this.updateObject) {
          if (this.updateObject[key as keyof IUser] === "") {
            delete this.updateObject[key as keyof IUser];
          }
        }

        Object.keys(this.updateObject.name).map((key) => {
          this.updateObject.name[key as keyof typeof this.updateObject.name] ===
          ""
            ? delete this.updateObject.name[
                key as keyof typeof this.updateObject.name
              ]
            : false;
        });

        // If user is  updated
        if (this.isUpdatedUser) {
          ...
          this.userNoUpdatedText =
            "User not updated, please fill at least one input";
          console.log(this.userNoUpdatedText);
        }
      }
    },
  },
